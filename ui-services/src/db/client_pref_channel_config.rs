use crate::{app_errors::MyError, models::{ClientPrefChannelConfig, }};
use deadpool_postgres::Client;

// // ***************************** ClientPrefChannelConfigs ****************************************
// A get for client pref channel config is not needed as this comes from the client preferences
...
//) -> Result<Vec<ClientPrefChannelConfig>, MyError> {
    ...
    for item in upsert_list {
        if item.client_pref_channel_config_id == -1 {
            client.query(&insert_stmt, &[&item.client_preference_id, &item.channel.channel_id, &item.selected_permitted,]).await.unwrap();
        } else {
            client.query(&update_stmt, &[&item.client_preference_id, &item.channel.channel_id, &item.selected_permitted, &item.client_pref_channel_config_id]).await.unwrap();
        }
    }
